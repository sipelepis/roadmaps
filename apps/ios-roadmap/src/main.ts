...
function shell(active: string, body: string, side = '') {
  ...
  app.innerHTML = `${SPRITE}
    <header class="nav">
      <a class="brand" href="#/" aria-label="iOS Roadmap home"><span class="mark">IO</span><span class="brand-name">Roadmap</span></a>
...
function home() {
  ...
  shell('home', `
    <section class="hero">
      <h1>Learn <em>iOS</em> the way it fits together.</h1>
      <p class="lead">${nodes.length} modules laid out by what they build on, each with an article, a live playground, and exercises with tests you solve right here in the browser. Logic you can run here in Swift and SwiftUI, screens you build in Xcode. No install.</p>
      <div class="actions">
