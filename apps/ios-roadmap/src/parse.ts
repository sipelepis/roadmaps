...
/* A problem is either solved in the editor (starter + tests, checked by the compiler) or built in the IDE
   (a checklist plus a reference solution to reveal), because a screen cannot be compiled in the browser. */
export interface Problem { title: string; html: string; starter: string; tests: string; solution: string; checklist: string[]; uses: string[]; hints: string[]; tips: string[]; docs: string[] }
...
// Article = everything before "## Exercises". Each "### ..." after it is a problem: either a ```<lang> starter```
// plus a ```<lang> test``` block, or a "#### Build it" checklist plus a ```<lang> solution``` block for the ones
// you build in the IDE. Then optional "#### Uses" (links to the sections it relies on), "#### Hints", "#### Tips"
// and "#### Docs" lists. ```<lang> playground``` in the article feeds the live editor.
export function parse(id: string, md: string): Module {
  ...
  let list: 'uses' | 'hints' | 'tips' | 'docs' | 'checklist' | undefined
  for (const t of cut < 0 ? [] : tokens.slice(cut + 1)) {
    if (isH(t, 3)) { cur = { p: { title: marked.parseInline(t.text) as string, html: '', starter: '', tests: '', solution: '', checklist: [], uses: [], hints: [], tips: [], docs: [] }, tokens: [] }; problems.push(cur.p); list = undefined }
    else if (!cur) continue
    else if (isCode(t, 'starter')) cur.p.starter = t.text
    else if (isCode(t, 'test')) cur.p.tests = t.text
    else if (isCode(t, 'solution')) cur.p.solution = t.text
    else if (isH(t, 4) && /^(build it)$/i.test(t.text)) list = 'checklist'
