...
export function mountEditor(el: HTMLElement, value: string, name: string) {
  const model = editor.createModel(value, 'rust', Uri.parse(`file:///${name}.rs`))
...
export function colorize(root: ParentNode) {
  root.querySelectorAll('pre code').forEach(el => editor.colorize(el.textContent ?? '', 'rust', {}).then(html => { el.innerHTML = html }))
}
