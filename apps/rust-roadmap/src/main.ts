...
function home() {
  ...
  shell('home', `
    <section class="hero">
      <h1>Learn <em>Rust</em> the way it fits together.</h1>
      <p class="lead">${nodes.length} modules laid out by what they build on, from ownership and borrowing to traits, iterators, and threads. Each has an article, a live playground, and exercises with tests you solve right here in the browser. Real Rust, compiled by the official Rust Playground. No install.</p>
      <div class="actions">
...
const STAGES: Record<number, string> = { 0: 'Foundations', 1: 'Foundations', 2: 'Core language', 3: 'Core language', 4: 'Ownership', 5: 'Ownership', 6: 'Data and types', 7: 'Data and types', 8: 'Abstraction', 9: 'Abstraction', 10: 'Advanced Rust', 11: 'In practice' }
  ...
  if (m.playground) {
    ...
    const runPlay = async () => {
      ...
      try {
        ...
        if (!pre.childNodes.length) print('Ran without output. Add a println! to see something here.', 'muted')
