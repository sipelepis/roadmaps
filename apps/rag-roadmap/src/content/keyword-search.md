...
`bm25(query, docs, k1=1.5, b=0.75)` returns one score per document using the formula above and the `tokenize` from exercise 1 (redefine it in your solution). `N` is the number of documents, `df` how many contain the term, and `avg` the mean token count.
