...
import { mountEditor, run, disposeAll, colorize, onRunKey, syncEditorTheme, pythonReady, type TestResult } from './editor'
...
function shell(active: string, body: string, side = '') {
  ...
  app.innerHTML = `${SPRITE}
    <header class="nav">
      <a class="brand" href="#/" aria-label="RAG Roadmap home"><span class="mark">RAG</span><span class="brand-name">Roadmap</span></a>
...
function wireProblem(id: string, i: number, p: Problem) {
  ...
  const runTests = async () => {
    ...
    runBtn.innerHTML = `${icon('spin')}<span>${pythonReady ? 'Running…' : 'Loading Python…'}</span>`
...
function home() {
  ...
  shell('home', `
    <section class="hero">
      <h1>Learn <em>RAG</em> from the page to the answer.</h1>
      <p class="lead">${nodes.length} modules laid out by what they build on, from scanned pages and OCR through chunking, embeddings, retrieval, and grounded answers. Each has an article, a live Python playground, and exercises with tests you solve right here in the browser.</p>
      <div class="actions">
...
const STAGES: Record<number, string> = { 0: 'Foundations', 1: 'Foundations', 2: 'From documents to chunks', 3: 'From documents to chunks', 4: 'Vectors and search', 5: 'Vectors and search', 6: 'Answers you can trust', 7: 'Answers you can trust', 8: 'In practice' }
...
function modulePage(id: string) {
  ...
  if (m.playground) {
    ...
    const runPlay = async () => {
      ...
      if (!pythonReady) print('Loading Python…', 'muted')
      try {
        const { stdout, error } = await run(ed.getValue())
        pre.replaceChildren()
        if (stdout) print(stdout.trimEnd())
        if (error) print(error, 'error')
        if (!pre.childNodes.length) print('Ran without output. Add a print() to see something here.', 'muted')
