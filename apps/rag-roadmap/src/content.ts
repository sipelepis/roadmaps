...
// with a ```python starter``` and a ```python test``` block. ```python playground``` in the article feeds the live editor.
function parse(id: string, md: string): Module {
  ...
  for (const t of cut < 0 ? [] : tokens.slice(cut + 1)) {
    ...
    else if (isCode(t, 'python starter')) cur.p.starter = t.text
    else if (isCode(t, 'python test')) cur.p.tests = t.text
  ...
  return {
    ...
    html: marked.parser(body.filter(t => !isH(t, 1) && !isCode(t, 'python playground'))),
    playground: body.find(t => isCode(t, 'python playground'))?.text ?? '',
    problems,
  }
}
