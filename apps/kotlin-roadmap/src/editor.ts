...
export function mountEditor(el: HTMLElement, value: string, name: string) {
  const model = editor.createModel(value, 'kotlin', Uri.parse(`file:///${name}.kt`))
...
export function colorize(root: ParentNode) {
  root.querySelectorAll('pre code').forEach(el => editor.colorize(el.textContent ?? '', 'kotlin', {}).then(html => { el.innerHTML = html }))
}
