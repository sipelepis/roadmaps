...
export function mountEditor(el: HTMLElement, value: string, name: string) {
  const model = editor.createModel(value, 'go', Uri.parse(`file:///${name}.go`))
  const ed = editor.create(el, {
    ...
    scrollBeyondLastLine: false, lineNumbersMinChars: 3, padding: { top: 12, bottom: 12 }, tabSize: 4, insertSpaces: false,
...
export function colorize(root: ParentNode) {
  root.querySelectorAll('pre code').forEach(el => editor.colorize(el.textContent ?? '', 'go', {}).then(html => { el.innerHTML = html }))
}
