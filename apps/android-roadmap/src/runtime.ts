...
function assemble(code: string, tests: string) {
  // Whatever the learner's own code imports stays where they wrote it. Hoisting a second copy of the same import
  // is not a harmless duplicate to Kotlin: it reports the name as ambiguous and nothing compiles.
  const already = new Set([...code.matchAll(/^[ \t]*import[ \t]+[^\n]+$/gm)].map(m => m[0].trim()))
  const hoisted = new Set(['import org.junit.Test', 'import org.junit.Assert.*'].filter(i => !already.has(i)))
  const body = tests.replace(/^[ \t]*import[ \t]+[^\n]+$/gm, line => { if (!already.has(line.trim())) hoisted.add(line.trim()); return '' })
  const header = hoisted.size ? [...hoisted].join('\n') + '\n\n' : ''
  return { source: header + code + '\n\n' + body, head: header ? header.split('\n').length - 1 : 0, split: code.split('\n').length + 1 }
}
...
function collect(results: Record<string, Case[]>, tests: { name: string; label: string }[], at: Layout): TestResult[] {
  ...
  return tests.map(({ name, label }) => {
    ...
    if (c.comparisonFailure) {
      // JUnit fills these for strings; for other types they come back empty and the message carries the values.
      const { expected, actual, message } = c.comparisonFailure
      if (expected || actual) { res.expected = expected; res.actual = actual }
      res.error = message
    } else if (c.exception) {
