...
function shell(active: string, body: string, side = '') {
  ...
  app.innerHTML = `${SPRITE}
    <header class="nav">
      <a class="brand" href="#/" aria-label="Android Roadmap home"><span class="mark">AN</span><span class="brand-name">Roadmap</span></a>
...
/** Nothing here ran, so a task says "Done" rather than borrowing the word a passing test earned. */
const taskBadge = (done: boolean) => done
  ? `<span class="badge passed">${icon('check')}Done</span>`
  : `<span class="badge todo">To build</span>`

/** A build task: no compiler can check a screen, so it gets a checklist, the reference solution, and your word. */
function taskHtml(id: string, i: number, p: Problem, showTitle: boolean) {
  const stat = `<span class="status" id="status-${i}">${taskBadge(passed(id, i))}</span>`
  return `<section class="problem task" aria-labelledby="ph-${i}">
    ...
    <p class="task-note">${icon('book')}Build this one in the IDE. Nothing here compiles it, so the checklist and the reference solution are the marking scheme.</p>
    ${p.checklist.length ? `<section class="checklist"><h4>Build it</h4><ul>${p.checklist.map((c, k) => `<li><label><input type="checkbox" data-check="${i}-${k}"><span>${c}</span></label></li>`).join('')}</ul></section>` : ''}
    ${tray(p)}
    <div class="bar">
      <button class="primary" data-done="${i}">${icon('check')}<span>Mark as done</span></button>
      ${p.solution ? `<details class="tests"><summary>Show a reference solution</summary><pre><code>${esc(p.solution)}</code></pre></details>` : ''}
    </div>
  </section>`
}
...
function problemHtml(id: string, i: number, p: Problem, showTitle = true) {
  ...
  if (!p.tests) return taskHtml(id, i, p, showTitle)
...
/** Build tasks are marked done by hand; the ticks are remembered so a half-finished screen keeps its place. */
function wireTask(id: string, i: number, p: Problem) {
  const stat = document.getElementById(`status-${i}`)!
  app.querySelectorAll<HTMLInputElement>(`[data-check^="${i}-"]`).forEach(box => {
    const key = `check:${id}:${i}:${box.dataset.check!.split('-')[1]}`
    box.checked = !!localStorage.getItem(key)
    box.onchange = () => box.checked ? localStorage.setItem(key, '1') : localStorage.removeItem(key)
  })
  const btn = app.querySelector<HTMLButtonElement>(`[data-done="${i}"]`)!
  const paint = () => {
    const done = passed(id, i)
    stat.innerHTML = taskBadge(done)
    btn.innerHTML = `${icon(done ? 'reset' : 'check')}<span>${done ? 'Mark as not done' : 'Mark as done'}</span>`
  }
  btn.onclick = () => {
    passed(id, i) ? localStorage.removeItem(passedKey(id, i)) : localStorage.setItem(passedKey(id, i), '1')
    paint(); count(app.querySelector('.pill .num')!, exercises.filter(e => passed(e.id, e.i)).length)
  }
  paint()
}

function wireProblem(id: string, i: number, p: Problem) {
  if (!p.tests) return wireTask(id, i, p)
...
function home() {
  ...
  shell('home', `
    <section class="hero">
      <h1>Learn <em>Android</em> the way it fits together.</h1>
      <p class="lead">${nodes.length} modules laid out by what they build on, each with an article, a live playground, and exercises with tests you solve right here in the browser. Logic you can run here in Kotlin and Jetpack Compose, screens you build in Android Studio. No install.</p>
      <div class="actions">
...
const STAGES: Record<number, string> = { 0: 'Foundations', 1: 'Foundations', 2: 'Building screens', 3: 'Building screens', 4: 'Building screens', 5: 'App architecture', 6: 'App architecture', 7: 'Data and wiring', 8: 'Quality', 9: 'Shipping', 10: 'In practice' }
