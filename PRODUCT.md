...
This file covers both apps in the monorepo, `apps/ts-roadmap` and `apps/py-roadmap`. They share one shell, one content format, and one purpose; only the language and its runtime differ. Facts marked *(inferred)* come from the repository and launch copy rather than a confirmed answer from the owner.
